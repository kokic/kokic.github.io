    ...
    redraw() {
        ...
        for (let i = this.style.level - 1, cut = true; i > 0; --i, cut = !cut) {
            new DOM.SVGElement("path", {
                ...
                "stroke-width": `${(i - (cut ? 1 : 0)) * CONSTANTS.STROKE_WIDTH
        ...
        new DOM.SVGElement("path", {
            ...
            stroke: "black",
            "stroke-width": edge_width + CONSTANTS.BACKGROUND_PADDING * 2,
            "stroke-dasharray": `${arclen_to_start + this.style.shorten.tail + shorten.start + ENDPOINT_PADDING} ${arclen_to_end - (arclen_to_start + shorten.start + this.style.shorten.tail
        ...
        return new DOM.SVGElement(tag_name, {
            ...
            transform: `translate(${origin.x} ${origin.y}) ${
                ...
                this.label.alignment === CONSTANTS.LABEL_ALIGNMENT.OVER ? "" :
                    `rotate(${-rad_to_deg(angle)} ${this.label.size.width / 2} ${this.label.size.height / 2})`
                }`,
        });
    }
