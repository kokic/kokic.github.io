...
> [!note]
> 注意 $\Delta = t^2 - 4t_0$ 本身要求了底域特征 $\operatorname{char}(k) \ne 2$, 否则 $4 = 0$, $\Delta$ 必为平方.
...
也就是 $p^2 = (t_1^2 - 4t_0)q^2$, 等式两边是 $k[t_0,t_1]$ 中的多项式, 所以 $\Delta$ 整除 $p^2$. 注意 $k[t_0,t_1]$ 是唯一分解整环 UFD, 此时不可约元一定是素元, 由 $\Delta$ 是不可约多项式知 $\Delta$ 整除 $p$, 因此存在 $p_1 \in k[t_0,t_1] $ 使得 $p = \Delta p_1$. 代入 $p^2 = \Delta q^2$, 因为 $\Delta \ne 0$, 可在整环中消去 $\Delta$, 得
...
> [!note]
> 这个论证实际上不必如此繁琐, 可直接把 $\Delta$ 看成关于 $t_0$ 的一次多项式 $t_1^2-4t_0\in k[t_1][t_0]$, 它对 $t_0$ 是一次的, 因此在这个环里已经不可约. 
